/** What you type on the ops login. Auth still needs a real email underneath. */
...
export function resolveAdminAuthEmail(login: string): string {
  const n = normalizeAdminEmail(login);
  if (n === ADMIN_LOGIN || n === ADMIN_AUTH_EMAIL) return ADMIN_AUTH_EMAIL;
  return login.trim();
}
